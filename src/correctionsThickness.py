...
continue_from = config.get('thickness', 'start_from')
...
logger = logging.getLogger("corrections&Thickness")
...
for bone in onlyfiles_stl:
    ...
    # reference bone does not need to be processed here
        
        if len(continue_from)>0:
            # if previous execution failed and we want to resume since the last failure
            logger.debug(VisualRecord("JUMPED BONE:  %s" %(i)))
            if bone.replace(".stl","")==continue_from:
        ...
        # processed folder is reached            
            ...
            #load leg side
            #linux
            side_path=output_path+"side/"+f"{bone.replace('.stl','')}.txt"
            #windows
            #side_path=output_path+"\\side\\"+f"{bone.replace('.stl','')}.txt"  
            with open(side_path,"r") as f:
                legside=f.read()    
            logger.debug(VisualRecord(">>> REFERENCE BONE SIDE IS:  %s" %(referencelegside)))
            logger.debug(VisualRecord(">>> PROCESSED BONE SIDE IS:  %s" %(legside)))
            ...
            """ Lets comment this part, comparing metadata seems to be more robust"""
            """
