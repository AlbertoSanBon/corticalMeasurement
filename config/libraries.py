# -*- coding: utf-8 -*-
"""
Created on Sat Jun 18 17:05:36 2022
...
def find_bounding_box(img, hu=True,threshold=200, display=True):
    """
    Introduction
    ------------
    ...
    if display:
        ...
        fig.suptitle("Bounding box detection", fontsize=16)
...
def find_bounding_box_sample_stack(img, hu=True, show_box=True, threshold=200, rows=10, cols=10, start_with=1, show_every=3):
    """
    Introduction
    ------------
...
def make_bonesmask(img, kernel_preErosion, kernel_firstDilation, kernel_firstErosion, hu=True, threshold=200, display=False, extract=[], size=16):
    """
    Introduction
    ------------
...
def show_cuts(array_thickness, cortesG, num_views, spacing, origin):
    ...
    for i in range(1, num_views+1):
        try:
            plt.subplot(num_views//4+1, num_views//3,i)              # Number of columns in the plot
...
def orientation_slice(image, represent=False):
    ...
    cv2.drawContours(image_rgb, contours, index, (255,255,255), 2)
    
    # Find the orientation of each shape
    vector=getOrientationMainVector(contours[index], image_rgb)
...
def getOrientationMainVector(pts, img):
    """
    Introduction
    ------------
    Obtain the main vector with the principal component.
    ...
    """
    ## Es parecido al anterior pero devuelve sólo el vector con la componente principal
    sz = len(pts)
    ...
    cv2.circle(img, cntr, 3, (255, 255, 0), 1)
    p1 = (cntr[0] + 0.02 * eigenvectors[0,0] * eigenvalues[0,0], cntr[1] + 0.02 * eigenvectors[0,1] * eigenvalues[0,0])
    drawAxis(img, cntr, p1, (255, 255, 0), 3)
    
    # Label with the rotation angle
    label = "Vector: ({:.4f},{:.4f})".format(eigenvectors[0,0],eigenvectors[0,1]) 
    #textbox = cv.rectangle(img, (cntr[0], cntr[1]-25), (cntr[0] + 250, cntr[1] + 10), (255,255,255), -1)
    cv2.putText(img, label, (cntr[0]-30, cntr[1]-100), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 0), 1, cv2.LINE_AA)
